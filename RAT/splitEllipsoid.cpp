...
namespace RAT
{
  static void binary_expand_op(real_T in1[50], int32_T in2, const real_T
...
namespace RAT
{
                      ...
                      int32_T VE2_size[2], real_T *nosplit)
  {
    ...
    if (u.size(0) < 2.0 * (static_cast<real_T>(u.size(1)) + 1.0)) {
      ...
      if ((static_cast<uint32_T>(r.size(0)) < static_cast<uint32_T>(u.size(1)) +
           1U) || (static_cast<uint32_T>(r1.size(0)) < static_cast<uint32_T>
                   (u.size(1)) + 1U)) {
        if (DEBUG != 0.0) {
          ...
          if ((flag1 != 0.0) || (flag2 != 0.0)) {
            ...
            for (int32_T i{0}; i < N; i++) {
              ...
              if (u.size(1) == mu1.size(1)) {
                ...
                coder::internal::blas::mtimes(r2, b_u, (real_T *)&VE1_tmp_data,
                  VE1_tmp_size);
              } else {
                c_binary_expand_op((real_T *)&VE1_tmp_data, VE1_tmp_size, r2, u,
                                   i, mu1);
              }

              coder::internal::blas::mtimes(VE1_data, VE1_size, (const real_T *)
                &VE1_tmp_data, VE1_tmp_size, (real_T *)&tmp_data, tmp_size);
              if (u.size(1) == mu2.size(1)) {
                b_u.set_size(mu2.size(0), u.size(1));
                for (int32_T b_i{0}; b_i < iindx; b_i++) {
                  c_loop_ub = mu2.size(0);
                  for (int32_T i1{0}; i1 < c_loop_ub; i1++) {
                    b_u[b_u.size(0) * b_i] = u[i + u.size(0) * b_i] -
                      mu2[mu2.size(0) * b_i];
                  }
              ...
              coder::internal::blas::mtimes(VE2_data, VE2_size, (const real_T *)
                &VE1_tmp_data, VE1_tmp_size, (real_T *)&b_tmp_data, b_tmp_size);
              if (b_tmp_size[0] == 1) {
                tmp_size_idx_0 = tmp_size[0];
              } else {
                tmp_size_idx_0 = b_tmp_size[0];
              }
              ...
              d_tmp_data.set(&c_tmp_data, tmp_size_idx_0, tmp_size_idx_1);
              if (coder::internal::ifWhileCond(d_tmp_data)) {
                m1++;
                c_loop_ub = u.size(1);
                for (int32_T b_i{0}; b_i < c_loop_ub; b_i++) {
            ...
            if (static_cast<int32_T>(m1) < 1) {
              loop_ub = 0;
            } else {
              loop_ub = static_cast<int32_T>(m1);
            }
            ...
            if (static_cast<int32_T>(m2) < 1) {
              loop_ub = 0;
            } else {
              loop_ub = static_cast<int32_T>(m2);
            }

            u2.set_size(loop_ub, u2new.size(1));
            b_loop_ub = u2new.size(1);
            for (int32_T b_i{0}; b_i < b_loop_ub; b_i++) {
              for (int32_T i1{0}; i1 < loop_ub; i1++) {
