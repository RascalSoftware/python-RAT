...
class Project(BaseModel, validate_assignment=True, extra="forbid", arbitrary_types_allowed=True):
    ...
    backgrounds: ClassList = ClassList(
        RAT.models.Background(name="Background 1", type=TypeOptions.Constant, value_1="Background Param 1"),
    )
    ...
    resolutions: ClassList = ClassList(
        RAT.models.Resolution(name="Resolution 1", type=TypeOptions.Constant, value_1="Resolution Param 1"),
    )
    ...
    def model_post_init(self, __context: Any) -> None:
        ...
        if "Substrate Roughness" not in self.parameters.get_names():
            self.parameters.insert(
                0,
                RAT.models.ProtectedParameter(
                    name="Substrate Roughness",
                    min=1.0,
                    value=3.0,
                    max=5.0,
                    fit=True,
                    prior_type=RAT.models.Priors.Uniform,
                    mu=0.0,
                    sigma=np.inf,
                ),
            )
    ...
    def check_protected_parameters(self) -> "Project":
        ...
        for class_list in parameter_class_lists:
            protected_parameters = [
                param.name for param in getattr(self, class_list) if isinstance(param, RAT.models.ProtectedParameter)
            ]
