...
namespace RAT
{
  static void cast(const ::coder::array<cell_wrap_21, 1U> &r, ::coder::array<
                   cell_wrap_8, 1U> &r1);
}
...
namespace RAT
{
  ...
  namespace nonPolarisedTF
  {
    namespace customLayers
    {
      ...
      {
        ::coder::array<cell_wrap_21, 1U> tempAllLayers;
        ...
        for (int32_T b_i{0}; b_i < i; b_i++) {
          ...
          backSort(contrastBackgrounds[b_i], contrastQzshifts[b_i],
                   ...
                   backgroundParam, qzshifts, scalefactor, bulkIn, bulkOut, res,
          ...
          if ((!std::isnan(x.re)) && (!std::isnan(x.im))) {
            ...
            callCppFunction(b_params, thisBulkIn, thisBulkOut,
                            static_cast<real_T>(b_i) + 1.0, (const char_T *)((::
              coder::array<char_T, 2U> *)&customFiles[static_cast<int32_T>(d) -
              1].f1)->data(), iv, b_thisContrastLayers, &allRoughs[b_i]);
          ...
          if (!useImaginary) {
            applyHydrationReal(thisContrastLayers, thisBulkIn, thisBulkOut);
          } else {
            applyHydrationImag(thisContrastLayers, thisBulkIn, thisBulkOut);
          }
