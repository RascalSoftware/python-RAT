...
namespace RAT
{
  static void b_binary_expand_op(::coder::array<boolean_T, 1U> &in1, const
    cell_27 *in2, const ::coder::array<real_T, 1U> &in3, real_T in4);
  ...
  static void binary_expand_op(::coder::array<creal_T, 1U> &in1, const ::coder::
    ...
    array<real_T, 1U> &in6);
  static void c_normalizeFunction(const ::coder::array<real_T, 1U> &x, const ::
    coder::array<real_T, 2U> &sldProfile, ::coder::array<real_T, 1U> &y);
...
namespace RAT
{
    ...
    2U> &sldProfile)
  {
    ...
    c_normalizeFunction(b_newDataPoints, sldProfile, r4);
    trueCount = r4.size(0);
    for (int32_T i{0}; i < trueCount; i++) {
    ...
    coder::array<real_T, 2U> &sldProfile, ::coder::array<real_T, 1U> &y)
  {
    ...
    for (int32_T b_i{0}; b_i < i; b_i++) {
      ...
      // [newValues{:}] = func(x(i));
      // y(i,:) = cell2mat(newValues);
      b_SLDFunction(x[b_i], sldProfile, RATMainTLSThread->f1.tmp_data, &tmp_size);
      y[b_i] = RATMainTLSThread->f1.tmp_data[0];
    }
                  ...
                  startDomain[2], real_T minAngle, real_T nPoints, cell_27 *out)
  {
    ...
    // func = @(x) normalizeFunction(x,sldProfile,vectorizable);
    //  Evaluate the input function on the initial set of points.
    c_normalizeFunction(newDomain, sldProfile, hiVal);
