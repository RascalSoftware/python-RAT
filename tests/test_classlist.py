...
class TestInitialisation(object):
    ...
    def test_identical_names(self, input_list: Sequence) -> None:
        """If we initialise a ClassList with input objects with identical "name" fields,
...
def test_setitem_same_name(two_name_class_list: 'ClassList', new_values: Dict) -> None:
...
def test_append_same_name(two_name_class_list: 'ClassList', new_values: Dict) -> None:
...
def test__validate_name(two_name_class_list: 'ClassList', input_dict: Dict) -> None:
    """We should not raise an error if the input values do not contain a name defined in an object in the ClassList."""
    assert two_name_class_list._validate_name(input_dict) is None


@pytest.mark.parametrize("input_dict", [
    ({'name': 'Alice'}),
])
def test__validate_name_not_unique(two_name_class_list: 'ClassList', input_dict: Dict) -> None:
    ...
    with pytest.raises(ValueError, match=(f"Input arguments contain the name '{input_dict['name']}',"
                                          f" which is already specified in the ClassList")):
        two_name_class_list._validate_name(input_dict)
...
def test__get_item_from_name(two_name_class_list: 'ClassList', name: str, expected_output: Union[object, str]) -> None:
    ...
    """
    assert two_name_class_list._get_item_from_name(name) == expected_output
